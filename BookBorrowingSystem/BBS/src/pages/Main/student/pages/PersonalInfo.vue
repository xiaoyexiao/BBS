...
export default {
  ...
  methods: {
    ...
    defer (row, table) {
      this.$confirm('延期还书?(可延长30天)', '提示', {
        ...
        this.$axios.get('http://localhost:8080/reletBook', {
          params: {
            order: row.order
          }
        }).then((response) => {
  ...
  mounted () {
    this.getTableData()
    this.$axios.post('http://localhost:8080/getStudent', {
      account: this.$store.state.id
    }).then((response) => {
      // console.log(response.data.data)
      this.personData = response.data.data
    })
    this.$axios.get('http://localhost:8080/borrowingBook', {
      params: {
        account: this.$store.state.id
      }
    }).then((response) => {
    ...
    this.$axios.get('http://localhost:8080/recordNumber', {
      params: {
        account: this.$store.state.id
      }
    }).then((response) => {
