...
export default {
  ...
  methods: {
    ...
    clickStar (index) {
      ...
      if (!this.uploadData[newIndex].star) {
        this.$axios.get('http://localhost:8080/saveCollect', {
          params: {
        ...
        this.$axios.get('http://localhost:8080/deleteCollect', {
          params: {
            account: this.$store.state.id,
            bookID: this.uploadData[newIndex].book.no
          }
  ...
  // 加载组件时更新表单
  mounted () {
    this.$axios.get('http://localhost:8080/countCategories', {
      params: {}
    }).then((response) => {
    ...
    this.$axios.get('http://localhost:8080/getCollections', {params: {
      account: this.$store.state.id
    }})
      .then((res) => {
